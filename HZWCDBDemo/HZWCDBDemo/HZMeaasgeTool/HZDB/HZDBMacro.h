 ...
 *  @return 一个搞定好的方法名
 */
#define HZDBMacroH(name) + (instancetype)shared##name;
 ...
 *  @return 单利
 */
#define HZDBMacroM(name) \
static HZDB *instance_ = nil;\
+ (instancetype)shared##name{\
static dispatch_once_t onceToken;\
dispatch_once(&onceToken, ^{\
NSString *path = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject];\
path = [path stringByAppendingString:@"/MyDataBase"];\
instance_ = [[self alloc] initWithPath:path];\
});\
return instance_;\
}\
